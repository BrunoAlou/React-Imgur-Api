...
const GalleryList = () => {
	...
	const displayGallery = () => {
		if (response && response['data']) {
			return (
				<div 
				className={`ig-grid ig-items ${(galleryParams.keyValues.amount)}`}>
					{response['data'].map((item, index) => (
						<button
							type="button"
							className="ig-item-button"
							key={`${item.id}-${index}`}
							ref={(response['data'].length - 5) === index ? lastItemFromGalleryRef : null}
	...
	return (
		<Container maxWidth="xl" className="ig-gallery-list">
